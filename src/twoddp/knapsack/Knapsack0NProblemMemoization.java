...
public class Knapsack0NProblemMemoization implements KnapsackProblem{
    ...
    public Knapsack0NProblemMemoization(Integer knapsackSize, List<Integer> weights, List<Integer> profits) {
    ...
    private Integer getMaxProfit(int index, Integer knapsackSize) {
        ...
        if (knapsackSize - this.weights.get(index) >= 0) {
            ans = this.profits.get(index)
                    + getMaxProfit(index, knapsackSize - this.weights.get(index));
        }
